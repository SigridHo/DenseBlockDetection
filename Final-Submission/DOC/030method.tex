...
\Output{$k$ dense block tables and a report table}
 ...
 \For{$i$ in range($k$)}{
  $MassR$ \gets Mass($R$) \\
  $Bn$ \gets find\_single\_block($R$,$Rn$,$MassR$,$density$)        \#See Algorithm 2\\
...
\Output{tables of attribute values forming a dense block}
 ...
 $MassB$ \gets $MassR$ \\
 CopyTable($Bn$, $Rn$) \\
 $BestDensity$ \gets DensityMeasure($MassB$, $Bn$, $MassR$, $Rn$)   \#See Algorithm 3 \\
 $r$,$Best\_r$ \gets 1 \\
 TableDropCreate($OrderTable$)\\
 \While{BlockNotEmpty($B$)}{
    \For{$n$ in range($N$)}{
    DistinctAttributeValue($Bn$)
    } 
    TableDropCreate($AttValMassTable$)\\
    AttValMass($AttValMassTable$)\\
    $i$ \gets SelectDimension() \#See Algorithm 4 \\
    $threshold$ \gets $MassB$ / $MassB_i$ \\
    SelectValuesToRemove($D_i$)\\
    CopyTable($D_static$, $D_i$)\\
    \While{TableNotEmpty($D_i$)}{
        $a$,$MassB_{a,i}$ \gets FetchFirstRow($D_i$)\\
        DeleteRows($D_i$) \# Delete all rows with same $a$ and  $MassB_{a,i}$\\
        DeleteRows($B_i$) \# Delete $a$ from $B_i$\\
        $MassB$ \gets $MassB$ - $MassB_{a,i}$\\
        $density'$ \gets DensityMeasure($MassB$, $Bn$, $MassR$, $Rn$) \\
        InsertRow($OrderTable$) \# Insert $a$,$i$,$r$ into $OrderTable$ \\
        $r$ \gets {$r$ + $1$}\\
        \If{${{density'}>{BestDensity}}$}{
            $BestDensity$ \gets $density'$ \\
            $Best\_r$ \gets $r$\\
        }
    }
    UpdateBlock($B$, $D_i$)\\
...
\If{$method$=$Geometric$}{
 ...
 \If{$method$=$Suspiciousness$}{
    $ratio$ \gets ${MassB}/{MassR}$ \\
...
This algorithm support two policy to select dimension for attribute value removal in Algoithm 2. They are select\_dimension by cardinality and select\_dimension by density.\\
\begin{algorithm}[H]
\SetKwInOut{Input}{Input}
\SetKwInOut{Output}{Output}
\SetAlgoLined
...
\If{$policy$=$density$}{
    $BestDensity$ \gets -\infty\\
    $dim$ \gets 1\\
    \For{$i$ in range($N$)}{
        \If{BlockNotEmpty($B_i$)}{
        $threshold$ \gets $MassB$ / $MassB_i$ \\
        SelectValuesToRemove($D_i$)\\
        $delta$ \gets DcubeSum($D_i$)\\
        $MassB'$ \gets $MassB$-$delta$\\
        $B_i'$ \gets UpdateBlock($B_i$, $D_i$)\\
        Switch $B_i'$ and $B_i$\\
        $density'$ \gets DensityMeasure($MassB$, $Bn$, $MassR$, $Rn$) \\
        Switch $B_i'$ and $B_i$\\
        \If{${{density'}>{BestDensity}}$}{
