...
def cube_sql_update_dcube_marker(db_conn, dest_table, conditions, setting):
    cur = db_conn.cursor()
    conditions = " AND ".join(conditions)
    query = "UPDATE %s SET %s WHERE %s" % (dest_table, setting, conditions)
    cur.execute(query)
    db_conn.commit()                       
    cur.close() 
...
def cube_sql_fetch_firstRow_marker(db_conn, dest_table, cons):
    cur = db_conn.cursor()
    query = "SELECT a_value, attrVal_mass::text FROM %s WHERE %s LIMIT 1" % (dest_table, cons) 
    cur.execute(query)
    (a_value, attrVal_mass) = cur.fetchone()  # fetch the corresponding a_value and attrVal mass
    db_conn.commit()                     
    cur.close() 
